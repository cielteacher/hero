...
//合并迭代查表计算
//这里将一个字节合并计算
//CRC8 生成码:x^8+x^5+x^4+1 = 100110001
//CRC8 校验位初始值，因为有些数据高位可能为0导致CRC校验能力降低，所以先异或初始值
const uint8_t CRC8_INIT = 0xff;  
//表格，一个字节有256种可能（0~255），将所有可能的CRC结果提前列出来，减少计算
 ...
 * @param ucCRC8     CRC 初始值（通常传入 CRC8_INIT）
 * @return 计算得到的 8 位 CRC 值
 */
...
/**
 * @brief 验证 CRC8 校验和（数据末尾已附加 CRC 字节）
 * @param pchMessage 包含数据和 CRC 字节的缓冲区指针
 * @param dwLength   缓冲区总长度（数据长度 + 1 字节 CRC）
 * @return 1 表示校验成功，0 表示失败
 */
...
/**
 * @brief 在数据末尾追加 CRC8 校验字节
 * @param pchMessage 缓冲区指针，前 dwLength-1 字节为数据，最后一字节预留用于存放 CRC
 * @param dwLength   缓冲区总长度（数据长度 + 1 字节预留空间）
 */
